...
class Problem:
    ...
    def randomState(self):
        ...
        while i < len(solution):
            ...
            if (remainedCore >= self.jobs[i].core and remainedRam >= self.jobs[i].ram):
                ...
                i -= 1
            i += 1
        return State.State(self.timeToLive, solution)
    ...
    def initialStates(self, numberOfInitialStates):
        ...
        for k in range(numberOfInitialStates):
            ...
            initStates.append(State.State(self.timeToLive, solution))

        return initStates

    def nextStates(self, states):
        nxtStates = []
        for state in states:
            if state.timeToLive > 0:
                neighbourState = state.mutant(self.timeToLive, self , states[(int)(random.random() * len(states) ) ])
                if self.f(neighbourState) >= self.f(state):
                    nxtStates.append(neighbourState)
                else:
                    state.timeToLive -= 1
                    nxtStates.append(state)
